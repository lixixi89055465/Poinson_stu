...
print(hm1.additional_kwargs)

template = ChatPromptTemplate.from_messages(
    [sy,
    hm1
     # AIMessage(content="肘肘的奇妙冒险"),
     # HumanMessage(
     #     content="请问这个小说的内容是什么样的"),
     # AIMessage(
     #     content="某个20岁少年{name},每顿饭吃100个鸡蛋,每天3万大卡热量摄入,深蹲时靠香草味的蛋白屁做氮气加速,他的蛋白屁是真蛋白,就是未完全消化的蛋清混合气体,与地面反作用力,产生跳跃式加速深蹲,有时候蛋白屁里混合着小虾仁,会让速度加快,当速度接近光速的时候,因为爱因斯坦的狭义相对论,使时间静止,这时候{name}大喊一声:砸瓦鲁多,会让时间静止10秒,"),
     # HumanMessage(content="我想要创作一部这样的小说,内容有文采一点优雅绅士一点,大概200字")
     ])
msg = template.format(name="张三", count=200)
