...
prompt = ChatPromptTemplate.from_messages(
    [
        ...
        ("placeholder", "{chat_history}"),#提示词中要加入聊天记录占位
...
# 把AgentExecutor放在RunnableWithMessageHistory里面
from langchain_community.chat_message_histories import ChatMessageHistory

historyObj = ChatMessageHistory()
agent_executor_history = RunnableWithMessageHistory(agent_executor,
                                                    lambda session_id: historyObj,
                                                    # 这里传入了参数session_id,但是没使用,lambda只做产生匿名函数的作用
                                                    input_messages_key="message",  # 输入信息的占位符
                                                    history_messages_key="chat_history",  # 保存历史记录的东西
                                                    # output_messages_key="output"  # 这个输出是agent 打印结果的 输出
                                                    )


def call_agent(input: str):
    ai_msg = agent_executor_history.invoke({"message": input},
                                           config={"configurable": {"session_id": "001"}})
    print("ai_msg=", ai_msg)
    print("historyObj=", historyObj)


call_agent("你好,我的名字是肘肘")
call_agent("告诉我我的名字是什么?不要说你好")
call_agent("我叫什么名字?")
