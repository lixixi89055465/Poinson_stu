...
memory = SqliteSaver(conn)#创建一个sqlite保存器
graph = graph_builder.compile(memory)#编译之后才能调用

while True:
    keyboardInput = input("请输入文字:\n")
    if keyboardInput == "close":
        conn.close() #注意多线程模式下会多出来2个文件,有这2个文件无法用Navicate for sqlite打开数据库,必须手动关闭才行
        break
    ...
    snapshot = graph.get_state(config)  # 打印next节点 为空,是因为已经到END了,所以为空
    print("1 snapshot=", snapshot)
