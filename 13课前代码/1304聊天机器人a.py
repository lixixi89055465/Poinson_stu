...
while True:
    inputUser = input("请输入文字:")  # input()是一个内置函数，其作用是从标准输入（一般就是键盘）获取用户输入的内容。
    print("inputUser=", inputUser)
    result = graph.invoke({"messages": [HumanMessage(content=inputUser)]})  # 调用
    print("result=",result)
    for i in range(len(result["messages"])):
        msg = result["messages"][i]
        if type(msg) == HumanMessage:
            print("i=",i,"人类信息",result["messages"][i].content)
        #elif 是python中的else if
        elif type(msg) == AIMessage:
            print("i=",i,"AI信息",result["messages"][i].content)
#可视化图
