...
memory = MemorySaver()#重启以后会消失
graph = graph_builder.compile(checkpointer= memory)#编译之后才能调用
...
#                           {"configurable": {"thread_id": "1"}} #参数二设置线程id
#                           )  # 调用
#     print("ai_msg=",ai_msg)
...
# ai_msg = graph.invoke({"messages": [HumanMessage(content="我叫jojo")]},
#                       {"configurable": {"thread_id": "1"}}  # 参数二设置线程id
# #                       )  # 调用
# print("ai_msg=",ai_msg)
ai_msg = graph.invoke({"messages": [HumanMessage(content="我叫什么?")]},
                      {"configurable": {"thread_id": "2"}}  # 参数二设置线程id
                      )  # 调用
print("ai_msg=",ai_msg)
#可视化图
