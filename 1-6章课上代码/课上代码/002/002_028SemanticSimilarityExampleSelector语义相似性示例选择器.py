...
# SemanticSimilarityExampleSelector
# 策略：该选择器基于语义相似度来挑选示例。它会计算输入文本与每个示例文本之间的语义相似度得分，然后选择得分最高的 k 个示例。
# 特点：简单直接，能确保选择的示例与输入在语义上最为接近，适用于需要紧密围绕输入主题的场景。例如，在进行近义词查询时，
# 它会优先选择与输入词语语义最相似的示例。
# MaxMarginalRelevanceExampleSelector
# 策略：采用最大边际相关性（MMR）算法进行示例选择。MMR 算法不仅考虑示例与输入的相关性，还会考虑示例之间的多样性。
# 它在选择示例时，会尽量保证所选示例既与输入相关，又能覆盖不同的方面，避免选择过于相似的示例。
# 特点：能在保证相关性的同时，提高示例的多样性。在一些需要综合考虑多方面信息的场景中非常有用，比如文本生成、问答系统等，可避免模型输出过于单一。
#
#  适用场景差异
# SemanticSimilarityExampleSelector
# 适用于对语义相关性要求极高，需要聚焦于特定主题的场景。例如，在进行法律条文的解释、医学术语的查询等任务时，
# 需要选择与输入语义最接近的示例来确保准确性。
# MaxMarginalRelevanceExampleSelector
# 更适合需要综合多方面信息、提高输出多样性的场景。比如在创意写作、多领域知识问答等场景中，
# 选择多样化的示例可以帮助模型生成更全面、更丰富的内容。
