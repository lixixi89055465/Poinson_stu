# 事件循环（Event Loop）：要运行异步代码，你需要一个事件循环。asyncio库提供了事件循环的实现。
# 你可以使用asyncio.run()来运行最顶层的异步函数：
#asyncio.run() 创建和管理事件循环：事件循环是异步编程的核心，它负责调度和执行异步任务。
# asyncio.run 会自动创建一个新的事件循环，然后将传入的协程放入该事件循环中运行。
