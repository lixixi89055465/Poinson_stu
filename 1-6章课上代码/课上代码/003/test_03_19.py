...
llm_with_tools = llm.bind_tools(tools,tool_choice='whichBigger')
...
for tool in ai_msg.tool_calls:
    fun = {'add': add, 'whichBigger': whichBigger}[tool['name']]
    args = tool['args']
    print(args)
    if fun == add:
        add.invoke({"a": args["a"], "b": args["b"]})  # invoke是执行@tool的方法的,里面要一个字典
    if fun == whichBigger:
        whichBigger.invoke({"a": args["a"], "b": args["b"]})  # invoke是执行@tool的方法的,里面要一个字典
    # fun.invoke({'a': args['a'], 'b': args['b']})
