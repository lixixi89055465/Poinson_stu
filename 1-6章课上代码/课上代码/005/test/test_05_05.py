...
# chunk_overlap,重叠字符数,可以确保相邻的文本块之间有一定的连贯性，避免信息的丢失。
# 例如，在进行文本摘要或问答系统时，重叠的部分可以帮助模型更好地理解上下文，从而提高处理的准确性。
# 分割器,把loader读取过来的长文档,分解成小的短文档
splitter = RecursiveCharacterTextSplitter(chunk_size=40, chunk_overlap=20)
split_document = splitter.split_documents(docs)  # 分割器返回分割后的文档
...
for doc in split_document:
    ...
    print(doc)
print("0" * 100)
# HuggingFaceEmbeddings抱脸虫可能会出现卡死的状态
...
# 这里加一个打印,测试是否执行完vectorstore.as_retriever(),这部容易卡死,如果卡死,
# 就删除HuggingFaceEmbeddings ,让他从新下载
...
# 而并行的执行结果还是个字典,可以传给下面的prompt当做invoke的参数
import os
