...
embedding = HuggingFaceEmbeddings(model_name='BAAI/bge-m3', cache_folder='D://cache//bge-m3')
...
print(f'history_aware_retriever:,{history_aware_retriever}')
...
# 跟上面的历史链一样,也需要传入input和 chat_history,但是多了一个context,
# 作用是接收上下文和历史聊天记录,并且查询答案
# 创建问题链
question_chain = create_stuff_documents_chain(llm, question_prompt)
print(f'question_chain,{question_prompt}')
...
def get_session_history(session_id: str):  # 这个是传入的id
    ...
    if session_id not in store:  # 如果对应的id不同,就不保存到新的key里面,
        print(f'ChatMessageHistory:,{ChatMessageHistory()}')
        store[session_id] = ChatMessageHistory()
        # 在内存中实现聊天消息的历史记录。 将消息存储在内存列表中。
        print(f'store[session_id]:,{store[session_id]}')
    print('store:', store)
...
def chat_withHistory(shuoCi: str):
    return final_chain.invoke(
        {"input": shuoCi}, config={"configurable": {"session_id": "001"}}
    )
