...
def first_story(state: State):
    """大模型调用生成第一轮幽默的文案"""
    prompt = ChatPromptTemplate.from_template("写一段幽默的文案大概50字关于话题 {topic}的")
...
def third_story(state: State):
    ...
    prompt = ChatPromptTemplate.from_template("让文字押韵有文采,在每行文字最后一个字上的韵母尽量相同,124句相同,第三局可以是别的韵母: {seconed_story_str}")
    chain = prompt | llm | StrOutputParser()
