...
graph_builder = StateGraph(State)  # 创建工作流状态,这里,所有工作流只能处理messages,因为上面的State,
# 只有一个字段
# 这个作用是可以后续添加节点node等
import os
...
def fun1(state: State):
    ...
    # 进入fun1节点以后会执行llm.invoke(state["messages"])
    # 调用大模型,把结果放在字典 {"messages": }中返回,状态更新为返回了AiMsg的状态
