# -*- coding: utf-8 -*-
# @Time : 2026/1/24 19:51
# @Author : nanji
# @Site : 
# @File : 1310_非自定义的工具调用.py.py
# @Software: PyCharm
# @Comment :
...
class State(TypedDict):
    ...
    # 这个字段如果改了名字,那么调用工具会报错,需要用自定义的BasicToolNode修改这个字段
...
# 创建节点函数
def chat_node_cond(state: State):
...
graph_builder.add_node('chat_node', chat_node_cond)  # 添加节点
...
# ,参数tools=[tool]表示该节点关联的工具列表,ToolNode是 LangGraph 中用于处理工具调用的节点类,这个可以自己写,
# 但是代码太多用框架写好的就行
